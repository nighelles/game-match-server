...
class Game(protocol.Protocol):
	...
	def dataReceived(self,data): 
		...
		# GAME REQUEST		1:game:longitude:latitude:tolerance:
		# SEND DATA			2:data
		# NEW USER			3:username
		# LEAVE				4:
		...
		if (command == GAMEREQUEST):
			cmd,game,longitude,latitude,tolerance,endline = data.split(":")
			print "Game request for: " + game + " At: " + longitude + " -- " + latitude
			self.gameRequest(game,float(longitude),float(latitude),float(tolerance))
	...
	def gameRequest(self,game,longitude,latitude,tolerance):
		...
		if matchingRequests.count() != 0:
			...
			self.users[selectedRequest.name] = users[selectedRequest.name]
			users[selectedRequest.name].users[self.user.name] = self
			...
			print "No match found, created new match",newgame.longitude,newgame.latitude,self.user.name,game,newgame.id
